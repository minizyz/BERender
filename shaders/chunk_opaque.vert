// BERender - Chunk Opaque Vertex Shader (Vulkan GLSL -> SPIR-V)
#version 450
...
void main(){
    uint si=uint(gl_InstanceID);
    vec3 wp=inPos+sections[si].worldOffset;
    outWorldPos=wp;
    gl_Position=camera.viewProj*vec4(wp,1.0);
    outUV=inUV; outNormal=decodeNormal(inNormal);
    outAO=decodeAO(inAOLight); outBlockLight=decodeBlockLight(inAOLight); outBlockId=inBlockId;
}
