// BERender - Chunk Opaque Fragment Shader
#version 450
...
void main(){
    ...
    float ambient=0.3; float skyLight=1.0;
    float lighting=inAO*(ambient+(1.0-ambient)*max(skyLight,inBlockLight));
    vec3 color=mat.baseColor.rgb*lighting+mat.emissive;
    float dist=length(inWorldPos-camera.position);
    float fog=clamp(1.0-exp(-dist*0.008),0.0,1.0);
    color=mix(color,vec3(0.6,0.75,0.9),fog);
